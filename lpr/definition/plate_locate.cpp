...
bool CPlateLocate::verifySizes(RotatedRect mr)
{
    ...
    int min = 62 * 20 * m_verifyMin;  // minimum area 34 * 8
    int max = 62 * 20 * m_verifyMax;  // maximum area
                            ...
                            const Point2f center, const double angle)
{
    ...
    in_large.create(int(in.rows * (abs(float(angle/30))+1)), int(in.cols * (abs(float(angle/30))+1)), in.type());
                                ...
                                double &slope)
{
    ...
    // std::cout << "len[0]:" << len[0] << endl;
    // std::cout << "len[1]:" << len[1] << endl;
    // std::cout << "len[2]:" << len[2] << endl;
    ...
    if (maxlen - len[1] > nCols / 32 || len[1] - minlen > nCols / 32)
    {
        ...
        // std::cout<<"g:"<<g<<endl;
        // std::cout << "slope_can_1:" << slope_can_1 << endl;
        // std::cout << "slope_can_2:" << slope_can_2 << endl;
        // std::cout << "slope_can_3:" << slope_can_3 << endl;
        // if(g>=0)
...
int CPlateLocate::deskew(const Mat &src, const Mat &src_b, vector<RotatedRect> &inRects,
...
int CPlateLocate::sobelOper(const Mat &in, Mat &out, int blurSize, int morphW, int morphH)
{
    ...
    Mat grad_x;     /** Mat grad_x, grad_y;*/
    Mat abs_grad_x; /** Mat abs_grad_x, abs_grad_y;*/
...
int CPlateLocate::sobelFrtSearch(const Mat &src, vector< Rect_<float> > &outRects)
{
    ...
    for (size_t i = 0; i < first_rects.size(); i++)
    {
        ...
        if(0)
        {
            ...
            roi_rect.points(rect_point);
            for(int i=0; i<4; i++)
                line(src, rect_point[i], rect_point[(i+1)%4], Scalar(0,0,255), 1, 1);
            imshow("src", src);
        }
...
int CPlateLocate::plateSobelLocate(Mat src, vector<CPlate> &candPlates, int index)
{
    ...
    for (int i = 0; i < (int)bound_rects_part.size(); i++)
    {
        ...
        float width = x + bound_rect.width < src.cols ? bound_rect.width : src.cols - x;
        float height = y + bound_rect.height < src.rows ? bound_rect.height : src.rows - y;
    ...
    deskew(src, src_b, rects_sobel_all, plates, false, UNKNOWN);
