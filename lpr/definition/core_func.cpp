...
void searchWeakSeed(const std::vector<CCharacter> &charVec, std::vector<CCharacter> &mserCharacter, double thresh1,
                        double thresh2, const Vec4f &line, Point &boundaryPoint,
                        const Rect &maxrect, Rect &plateResult, Mat result,
...
void rotatedRectangle(InputOutputArray image, RotatedRect rrect, const Scalar &color,
                ...
                bool showDebug)
{
    ...
    for (int color_index = 0; color_index < 2; color_index++)
    {
        ...
        for (size_t index = 0; index < size; index++)
        {
            ...
            {
                Mat mserMat = adaptive_image_from_points(contour, rect, Size(char_size, char_size), Scalar(0, 0, 0), Scalar(255,255,255), 1);
                ...
                if (judegMDOratio2(image, rect, contour, result, 150, 0))
                {
                    CCharacter charCandidate;
        ...
        for (auto plate : plateVec)
        {
            ...
            if (verifyRotatedPlateSizes(platePos, 0))
            {
